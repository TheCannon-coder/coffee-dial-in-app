    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <View style={[styles.recipeCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <RecipeRow icon="package" label="Coffee" value={recipe.dose} />
          <RecipeRow icon="droplet" label="Water" value={recipe.water} />
          <RecipeRow icon="thermometer" label="Temperature" value={recipe.temp} />
          <RecipeRow icon="clock" label="Brew time" value={recipe.time} />
...
function RecipeRow({ icon, label, value }: { icon: string; label: string; value: string }) {
  const colors = useColors();
  return (
    <View style={rowStyles.row}>
      ...
      <Text style={[rowStyles.value, { color: colors.espresso, fontFamily: 'Fraunces_500Medium' }]}>{value}</Text>
    </View>
  );
}
