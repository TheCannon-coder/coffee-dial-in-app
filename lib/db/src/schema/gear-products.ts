...
export const GEAR_EXPERIENCE_LEVELS = ["beginner", "intermediate", "advanced"] as const;
export type GearExperienceLevel = typeof GEAR_EXPERIENCE_LEVELS[number];
...
export const gearProductsTable = pgTable("gear_products", {
  ...
  /** Constrained to beginner | intermediate | advanced via DB CHECK constraint */
...
export const insertGearProductSchema = createInsertSchema(gearProductsTable)
  .omit({ id: true, createdAt: true })
  .extend({
    experienceLevel: z.enum(GEAR_EXPERIENCE_LEVELS).default("beginner"),
  });
